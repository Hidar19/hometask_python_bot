...
from config import conn, cur, bot, KING_ID
...
Отлично! Теперь напишите последнюю букву 1 класса в формате: <i>в</i>.
Если 1 класс только один напишите а
Если есть 1е класс свяжитесь с разработчиком(@Hidar82)''',
                                     reply_markup=keyboard_teacher_cancel)
...
В новый учебный год вступают: \n{', '.join(new_classes)} классы
После завершения действия все классы будут смещены на 1''',
                         reply_markup=TeacherKeyboards.confirmation())
    ...
    for i in cur.execute("SELECT class, user_id FROM users").fetchall():
        if i[0][:2] == '10':
            if i[0][1:] in rus_alphabet:
                ...
                cur.execute(
                    "UPDATE users SET class=? WHERE user_id=?", ('11', i[1]))
        else:
            if i[0][1:] in rus_alphabet:
                cur.execute("UPDATE users SET class=? WHERE user_id=?",
                            (str(int(i[0][0]) + 1) + i[0][1:], i[1]))
            else:
                cur.execute("UPDATE users SET class=? WHERE user_id=?",
                            (int(i[0][0]) + 1, i[1]))
    conn.commit()
...
def new_year_teacher_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        agree_NewYear, text='agree', state=FSMNew_Year.agree)
