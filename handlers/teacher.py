...
async def teacher_menu(message: Message):
    if check_admin(message.from_user.username) is True:
...
async def cancel_menu(call: CallbackQuery, state: FSMContext):
    ...
    if call.from_user.id == KING_ID:
        await call.message.answer('Что вы хотите сделать?', reply_markup=TeacherKeyboards.king_admin_keyboard_generate(distant))
    elif check_admin(call.from_user.username) is True:
        await call.message.answer('Что вы хотите сделать?', reply_markup=TeacherKeyboards.teacher_keyboard_generate(distant))
