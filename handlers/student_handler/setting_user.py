...
async def setting(message: Message):
    person = cur.execute(
        "SELECT first_name, class FROM users WHERE user_id = ?", (message.from_user.id,)).fetchall()
    if len(person) == 0:
        await homework.homework_show(message, state=None)
    await message.answer(f'''<b>Настройки</b>
Класс: <i>{person[0][1]} класс,</i>
Обращение: <i>{person[0][0]}</i>''', reply_markup=StudentKeyboards.keyboard_setting())


async def cancel_setting(call: CallbackQuery, state: FSMContext):
    person = cur.execute(
        "SELECT first_name, class FROM users WHERE user_id = ?", (call.from_user.id,)).fetchall()
    await bot.answer_callback_query(call.id)
    await state.finish()
    await call.message.edit_text(f'''<b>Настройки</b>
Класс: <i>{person[0][1]} класс,</i>
Обращение: <i>{person[0][0]}</i>''', reply_markup=StudentKeyboards.keyboard_setting())
